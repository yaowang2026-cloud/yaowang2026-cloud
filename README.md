<p align="center">
  <a href="#english"><b>🇺🇸 English</b></a> |
  <a href="#中文"><b>🇨🇳 中文</b></a>
</p>

---

## English

### 🚀 About Me

- 📊 Data-driven analyst with strong background in econometrics & machine learning  
- 🤖 Interested in AI applications in business and decision-making  
- 📈 Focus on risk, uncertainty, and data-informed strategy  
- 💼 Target roles: Data Analyst | Business Analytics | Risk  

---

### ⚙️ Data & Analytics Toolkit

<p align="center">
<table align="center">
<tr>

<td align="center" width="120">
<img src="https://skillicons.dev/icons?i=python" width="60"/><br>
Python
</td>

<td align="center" width="120">
<img src="https://skillicons.dev/icons?i=mysql" width="60"/><br>
SQL
</td>

<td align="center" width="120">
<img src="https://cdn.worldvectorlogo.com/logos/r-lang.svg" width="60"/><br>
R
</td>

<td align="center" width="120">
<img src="https://cdn.worldvectorlogo.com/logos/tableau-software.svg" width="60"/><br>
Tableau
</td>

<td align="center" width="120">
<img src="https://img.icons8.com/color/48/power-bi.png" width="60"/><br>
Power BI
</td>

</tr>
</table>
</p>

---

### 📌 Featured Projects

#### 📉 IPO Underpricing & Credit Risk
- Built empirical model linking bond defaults to IPO performance  
- Applied **IV (2SLS)** + Fixed Effects  
- Dataset: **2692 IPOs (2014–2023)**  

#### 🍽️ Market Research & A/B Testing
- Improved email performance: **25% → 60% open rate**  
- Applied regression, clustering, and IPA  

#### 📊 Portfolio Optimization
- Mean-variance + Sharpe optimization  
- Monte Carlo simulation for risk  

---

### 📫 Contact

<p align="center">
  <a href="mailto:wycwinston@outlook.com">
    <img src="https://img.shields.io/badge/Email-contact-blue?style=for-the-badge&logo=gmail" />
  </a>
</p>

---

### ⚡ Beyond Work

🏂 Snowboard | 🎿 Ski | ⛳ Golf

---

## 中文

### 🚀 关于我

- 📊 数据分析方向，专注计量经济学与机器学习  
- 🤖 关注 AI 在商业中的应用  
- 📈 研究风险、不确定性与数据驱动决策  
- 💼 求职方向：数据分析 / 商业分析 / 风险  

---

### ⚙️ 技术栈

<p align="center">
<table align="center">
<tr>

<td align="center" width="120">
<img src="https://skillicons.dev/icons?i=python" width="60"/><br>
Python
</td>

<td align="center" width="120">
<img src="https://skillicons.dev/icons?i=mysql" width="60"/><br>
SQL
</td>

<td align="center" width="120">
<img src="https://cdn.worldvectorlogo.com/logos/r-lang.svg" width="60"/><br>
R
</td>

<td align="center" width="120">
<img src="https://cdn.worldvectorlogo.com/logos/tableau-software.svg" width="60"/><br>
Tableau
</td>

<td align="center" width="120">
<img src="https://img.icons8.com/color/48/power-bi.png" width="60"/><br>
Power BI
</td>

</tr>
</table>
</p>

---

### 📌 项目经历

#### 📉 IPO 抑价与信用风险
- 使用 IV（2SLS）+ 固定效应模型  
- 研究债券违约对 IPO 的影响  
- 数据规模：2692 家 IPO  

#### 🍽️ 市场调研与 A/B 测试
- 邮件打开率：**25% → 60%**  
- 使用回归、聚类与 IPA 方法  

#### 📊 投资组合优化
- 均值-方差 + Sharpe 优化  
- Monte Carlo 模拟风险  

---

### 📫 联系方式

<p align="center">
  <a href="mailto:wycwinston@outlook.com">
    <img src="https://img.shields.io/badge/Email-contact-blue?style=for-the-badge&logo=gmail" />
  </a>
</p>

---

### ⚡ 兴趣

🏂 单板滑雪 | 🎿 双板滑雪 | ⛳ 高尔夫

